...
namespace MGUI.Core.UI.XAML
{
    ...
    internal static class StyleResolver
    {
        public static void ValidateNamedStyles(IReadOnlyDictionary<string, Style> existingStyles, IReadOnlyDictionary<string, Style> incomingStyles)
        {
            ...
            foreach (KeyValuePair<string, Style> kvp in incomingStyles)
            {
                ResolveSetters(kvp.Key, kvp.Value, allStyles);
            }
        }

        public static IReadOnlyList<Setter> ResolveSetters(Style style, IReadOnlyDictionary<string, Style> stylesByName)
            => ResolveSetters(null, style, stylesByName);

        public static IReadOnlyList<Setter> ResolveSetters(string styleName, Style style, IReadOnlyDictionary<string, Style> stylesByName)
        {
            if (style == null)
            {
        ...
        public static IReadOnlyList<ResolvedStyle> ResolveExplicitStyles(string styleNames, MGElementType targetType, IReadOnlyDictionary<string, Style> stylesByName)
        {
        ...
        private static void ResolveSetters(string currentStyleName, Style currentStyle, IReadOnlyDictionary<string, Style> stylesByName, List<Setter> resolvedSetters, List<string> activePath)
        {
            ...
            try
            {
                ...
                {
                    if (stylesByName == null || !stylesByName.TryGetValue(currentStyle.BasedOn, out Style baseStyle))
                    {
